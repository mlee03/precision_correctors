...
contains
  ...
  subroutine init(self, name, dims)
    class(Field64), intent(inout) :: self
  ...
  pure subroutine assign_array(self, a)
    class(Field64), intent(in out) :: self
  ...
  pure function diffx(input_field)
    ...
    class(Field64), intent(in) :: input_field
  ...
  function gather(self, image)
    ...
    class(Field64), intent(in) :: self
    integer(int32), intent(in) :: image
    real(real64), allocatable :: gather_coarray(:,:)[:]
    real(real64) :: gather(self % dims(1), self % dims(2))
  ...
  pure subroutine set_gaussian(self, decay, ic, jc)
    class(Field64), intent(in out) :: self
    real(real64), intent(in) :: decay ! the rate of decay of gaussian
  ...
  pure type(Field64) function field_add_field(self, f) result(res)
    class(Field64), intent(in) :: self, f
    res = self
    res = self % data + f % data
  end function field_add_field

  pure type(Field64) function field_add_real(self, x) result(res)
    class(Field64), intent(in) :: self
  ...
  pure type(Field64) function real_add_field(x, self) result(res)
    class(Field64), intent(in) :: self
    real(real64), intent(in) :: x(:,:)
    res = self
    res = self % data + x
  ...
  pure type(Field64) function field_div_real(self, x) result(res)
    class(Field64), intent(in) :: self
    real(real64), intent(in) :: x
    res = self
    res = self % data / x
  ...
  pure type(Field64) function field_mult_array(self, x) result(res)
    class(Field64), intent(in) :: self
    real(real64), intent(in) :: x(:,:)
    res = self
    res = self % data * x
  ...
  pure type(Field64) function field_mult_real(self, x) result(res)
    class(Field64), intent(in) :: self
    real(real64), intent(in) :: x
    res = self
    res = self % data * x
  ...
  pure type(Field64) function field_mult_field(self, f) result(res)
    class(Field64), intent(in) :: self, f
    res = self
    res = self % data * f % data
  end function field_mult_field

  pure type(Field64) function array_mult_field(x, self) result(res)
    real(real64), intent(in) :: x(:,:)
    class(Field64), intent(in) :: self
    res = self
    res = self % data * x
  ...
  pure type(Field64) function field_sub_array(self, x) result(res)
    class(Field64), intent(in) :: self
    real(real64), intent(in) :: x(:,:)
    res = self
    res = self % data - x
  ...
  pure type(Field64) function field_sub_field(self, f) result(res)
    class(Field64), intent(in) :: self, f
    res = self
    res = self % data - f % data
  ...
  subroutine write(self, n)
    ...
    real(real64), allocatable :: gather(:,:)
